...
Group Discourse at Scale [Preprint]. arXiv. https://doi.org/10.48550/arXiv.2609.27059
```
...
11. ```organize_anonymize```: Replaces author usernames with persistent random IDs to safeguard Reddit users' privacy. The user-ID key is stored in a local SQL database. With parallelization via ```--slurm```, every author in the requested months must already hold an ID before anonymization starts. The resource handles this preparation automatically if needed. Local (non-```--slurm```) runs mint IDs on demand and need no warm-up.
...
Recreating the corpus writes terabytes of CSVs across long-running jobs. Storage faults and interrupted jobs can therefore leave damage that file sizes and timestamps do not reveal. The ```verify_integrity``` resource checks curated outputs by reading every byte. It has no fixed place in the resource order: it verifies the most advanced curated directory per the canonical order described above for the given ```--group``` and ```--type``` (preferring an anonymized ```_anon``` directory when one exists), or the directory passed with ```--input```. The checks follow from the directory: ```organize_anonymize``` outputs are reconciled row-for-row against their pre-anonymization source and every anonymized author ID is cross-referenced against the author map; ```organize_types``` outputs are reconciled against the comments and submissions they were merged from; ```filter``` and ```label``` outputs receive structural checks (header, NUL bytes, torn rows, column counts, timestamps).
```
...
### Label Generation
If the social-psychological labels provided alongside ISAAC work well for your use case, you can apply ```label``` resources to generate them for your new corpus. These tools are also available for use with independent data, coding-free, at [https://huggingface.co/spaces/BabakScrapes/isaac-classifiers](https://huggingface.co/spaces/BabakScrapes/isaac-classifiers). The location model is currently excluded from the tool to prevent misuse. Write [us](mailto:isaac.corpus.support@gmail.com) for potential research access.
