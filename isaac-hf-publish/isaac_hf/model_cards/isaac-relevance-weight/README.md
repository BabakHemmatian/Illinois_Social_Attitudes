...
- isaac
extra_gated_heading: "Request access to the ISAAC relevance classifier (body weight)"
extra_gated_prompt: >-
...
- name: isaac-relevance-weight
  ...
  - task:
    ...
    dataset:
      ...
      name: "ISAAC relevance annotations (weight) — Held-out 10% of the rated training sample"
      split: test
    metrics:
    - type: precision
...
# ISAAC relevance classifier — Body weight (thin vs. fat)

Binary classifier that decides whether an English text is **relevant to the
body weight social group distinction** — that is, whether it actually
...
REPO = "ISAAC-corpus/isaac-relevance-weight"
tokenizer = AutoTokenizer.from_pretrained(REPO)
model = AutoModelForSequenceClassification.from_pretrained(REPO).eval()

texts = ["My grandmother is tired of being told to lose weight."]
...
Each post was rated for relevance to the body weight distinction by **two
...
Interrater reliability on this distinction: *k* = 1,475 double-rated posts,
89.6% raw agreement, Cohen's κ = .690.
...
| --- | --- | --- | --- | --- |
| Held-out 10% of the rated training sample | 148 | .975 | 1.000 | .987 |

**Human agreement on the training data.** Two trained annotators rated
*k* = 1,475 posts: 89.6% raw agreement,
Cohen's κ = .690.
...
body weight data (*k* = 100)
found 4.0% of comments irrelevant under the stringent
...
Screening English social-media text for relevance to the body weight
...
general-purpose "is this about body weight" classifier for arbitrary
input.
...
* **Trained on human judgment, which varied.** Cohen's κ of .690 on the
