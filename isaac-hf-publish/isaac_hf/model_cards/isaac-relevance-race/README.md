...
- isaac
extra_gated_heading: "Request access to the ISAAC relevance classifier (race)"
extra_gated_prompt: >-
...
- name: isaac-relevance-race
  ...
  - task:
    ...
    dataset:
      ...
      name: "ISAAC relevance annotations (race) — Held-out 10% of the retraining sample"
      split: test
    metrics:
    - type: precision
  ...
  - task:
    ...
    dataset:
      type: isaac-relevance-annotations
      name: "ISAAC relevance annotations (race) — Held-out 10% of the initial training sample"
      split: test
    metrics:
    - type: precision
      name: Precision (relevant class)
...
# ISAAC relevance classifier — Race (White vs. Black)

Binary classifier that decides whether an English text is **relevant to the
race social group distinction** — that is, whether it actually
...
REPO = "ISAAC-corpus/isaac-relevance-race"
THRESHOLD = 0.6   # the ISAAC decision rule; do NOT use plain argmax

tokenizer = AutoTokenizer.from_pretrained(REPO)
model = AutoModelForSequenceClassification.from_pretrained(REPO).eval()

texts = ["My grandmother is the only Black woman on her block."]
...
Each post was rated for relevance to the race distinction by **two
...
Interrater reliability on this distinction: *k* = 1,498 double-rated posts,
92.6% raw agreement, Cohen's κ = .784.
...
| --- | --- | --- | --- | --- |
| Held-out 10% of the retraining sample | 40 | .786 | .917 | .846 |
| Held-out 10% of the initial training sample | 150 | 1.000 | .757 | .862 |
...
*k* = 1,498 posts: 92.6% raw agreement,
Cohen's κ = .784.
...
race data (*k* = 150)
found 8.0% of comments irrelevant under the stringent
rule and 5.3% under the lenient rule
(4.0% for submissions). That end-to-end figure, not
the F1 above, is the number to quote when describing corpus quality.

## Intended use

Screening English social-media text for relevance to the race
...
general-purpose "is this about race" classifier for arbitrary
input.
...
* **Trained on human judgment, which varied.** Cohen's κ of .784 on the
